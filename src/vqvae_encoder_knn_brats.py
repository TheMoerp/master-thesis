...
# create_unique_results_dir provided by common.brats_preprocessing
...
def main():
    ...
    parser.add_argument('--pretrained_weights', type=str, default='', help='Optional path to VQ-VAE weights (from vqvae_brats.py)')
    ...
    # Route outputs into unique subfolder under ./results
    ...
    if args.pretrained_weights and os.path.isfile(args.pretrained_weights):
        ...
        # Try to locate best VQ-VAE weights under root results/results_vqvae_brats*
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(script_dir) if os.path.basename(script_dir) == 'src' else script_dir
        results_root = os.path.join(project_root, 'results')
        try:
            if os.path.isdir(results_root):
                # Search for most recently modified folder matching results_vqvae_brats*
                candidates = []
                for d in os.listdir(results_root):
                    full = os.path.join(results_root, d)
                    if os.path.isdir(full) and d.startswith('results_vqvae_brats'):
                        candidates.append((os.path.getmtime(full), full))
                if candidates:
                    candidates.sort(reverse=True)
                    for _, folder in candidates:
                        ckpt = os.path.join(folder, 'best_vqvae_3d.pth')
                        if os.path.isfile(ckpt):
                            try:
                                state = torch.load(ckpt, map_location=cfg.device)
                                if _safe_load_encoder_weights(model, state):
                                    break
                            except Exception:
                                continue
        except Exception:
